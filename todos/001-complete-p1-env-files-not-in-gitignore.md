...
`.env.development` and `.env.production` are tracked by git and contain real API base URLs:
...
Neither file appears in `.gitignore`. This exposes real backend hostnames to anyone with repo access.
...
**Impact:** Any developer or attacker with repo access can query backend directly. Combined with leaked/weak auth token → data access.
...
**Pros:** Complete fix. **Cons:** Requires coordinating team to re-create local env files.
**Effort:** Small. **Risk:** Low.

### Option B: Keep in git with placeholder values
Replace real URLs with `https://your-backend.example.com` and commit.

**Pros:** Simple. **Cons:** Doesn't fix existing history if URLs were ever pushed. Doesn't solve the root pattern.
**Effort:** Small. **Risk:** Low.
...
- [ ] Git history audited — if URLs were pushed to remote, purge with filter-repo
