...
`tailwind.config.js` is a Tailwind v3 format config file. The project uses
`@tailwindcss/postcss` (v4 pipeline) with `@theme` in `globals.css` as the
authoritative config. The `tailwind.config.js` file is never read by the v4
pipeline.
...
The comment in the file says it is for reference only, but its presence is still
misleading.

## Proposed Solution

Delete `tailwind.config.js`. Any documentation value it provides can be replaced
with a comment in `globals.css` in the `@theme` block.
