...
Both `login` and `register` Server Actions call `fetch` without a timeout. If
the Laravel backend is slow or unresponsive, the Server Action hangs
indefinitely, blocking the user's request with no feedback.
...
When the timeout fires, fetch throws `DOMException: The operation was aborted`.
This should be caught and shown as a user-friendly error ("Request timed out,
please try again").

Consider adding this to `shared/lib/httpClient.ts` as well for consistency
