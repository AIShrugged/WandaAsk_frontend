...
The mobile responsiveness refactor changed `features/analysis/widgets/linear.tsx` from a data-adaptive grid to a fixed 4-column layout. On desktop, if the API returns fewer than 4 submetrics, cards are now left-aligned in a partial grid with empty cells instead of filling the row proportionally.
...
`MetricItem[]` has no length constraint at the type level. The backend controls submetric count.
