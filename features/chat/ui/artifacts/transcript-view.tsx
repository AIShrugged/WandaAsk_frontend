import type { TranscriptArtifact } from '@/features/chat/types';
...
export function TranscriptView({ data }: { data: TranscriptArtifact['data'] }) {
  ...
  if (entries.length === 0) {
    return <p className='text-sm text-muted-foreground text-center py-6'>No transcript</p>;
  }
        ...
        <div key={i} className='flex items-start gap-2.5'>
          ...
          <div className='flex-1 min-w-0'>
            <div className='flex items-baseline gap-2 mb-0.5'>
              ...
              <span className='text-xs text-muted-foreground tabular-nums'>{entry.timestamp}</span>
            </div>
