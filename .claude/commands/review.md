...
If $ARGUMENTS is provided, review the specified file or feature. Otherwise,
review all uncommitted changes (`git diff`).

Review checklist:

1. **FSD Architecture** — features isolated, no cross-feature imports, shared
   code in `shared/`
...
6. **Performance** — no unnecessary re-renders, proper memoization only when
   needed
...
9. **Error handling** — toast notifications for user-facing errors, proper
