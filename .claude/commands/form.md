...
The argument format is: `<feature>/<FormName>` (e.g., `auth/LoginForm` or `meeting/CreateMeetingForm`).

This creates a complete form setup:

1. **Zod schema** in `features/<feature>/model/` — validation rules using Zod v4 syntax
   - Use `z.email()` not `z.string().email()`
   - Use `z.literal(value, { error })` not `errorMap`
2. **Form component** in `features/<feature>/ui/` — Client Component with `'use client'`
