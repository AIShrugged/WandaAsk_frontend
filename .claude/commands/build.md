...
- Server/Client Component boundary violations (using hooks in Server Components,
  importing client modules in server context)
...
- Environment variables not in `next.config.ts` `env` or missing `NEXT_PUBLIC_`
  prefix
...
- Do not suppress errors with `@ts-ignore` or `eslint-disable` — fix the root
  cause
- If a fix requires architectural changes, describe the issue and proposed fix
  before applying
