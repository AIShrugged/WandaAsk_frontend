...
### OWASP Top 10
- **Injection**: Check for unsanitized user input in Server Actions, API routes, and dynamic queries
- **XSS**: Verify no `dangerouslySetInnerHTML` without sanitization (check `html-react-parser` usage)
- **Auth issues**: Check token storage, session management, middleware protection
- **SSRF**: Verify server-side fetch calls don't accept user-controlled URLs
- **Mass assignment**: Check that Server Actions validate/whitelist input fields with Zod
...
- Ensure API keys/secrets are only in server-side code (not in `NEXT_PUBLIC_*` env vars)
...
Report findings as: CRITICAL / HIGH / MEDIUM / LOW with specific file paths and fixes.
